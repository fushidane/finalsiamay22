...
<div class="d-flex justify-content-between gap-2 p-4" style="background-color: pink;">
    <div style="text-align: right; border: 1px solid #ccc; padding: 10px; border-radius: 5px; display: flex; align-items: center;">
        <form action="{{ route('laptop.import') }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column gap-2 shadow-sm p-3 bg-white rounded">
            ...
            <button type="submit" class="btn btn-primary btn-sm mt-2" style="margin-right: 5px;">Import CSV</button>
        </form>

        <a href="{{ route('scanner') }}" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-2" style="margin-right: 5px;">
        ...
        <a href="/laptops/csv-all" target="_blank" class="btn btn-primary rounded-2" style="margin-right: 5px;">CSV</a>
        <a href="/laptops/pdf" target="_blank" class="btn btn-primary rounded-2" style="margin-right: 5px;">Export</a>
    </div>
</div>
